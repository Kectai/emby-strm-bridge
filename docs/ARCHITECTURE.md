...
9. `GatewayTransport` forwards approved request headers, follows up to the configured redirect limit and retains the real client User-Agent. It also provides bounded partial responses to `GatewayFastSeekProbeClient`. A ticket lease validates redirect reuse, and a source-scoped candidate lets only the server-FFmpeg media request try the address validated during calibration with its own User-Agent. Candidate reuse requires an exact matching 206 range and length and otherwise resolves again from the original source. When relay concurrency is greater than one, probes leave one configured slot available for playback delivery.
10. Direct-client requests derive a memory-only context from the ticket depth, actual upstream resource, item, media source, STRM fingerprint, authorization binding and runtime-HMAC device binding, then separate it by method, User-Agent, `Accept` and `Accept-Language`. A missing device binding narrows the context to one ticket. A reference-counted, generation-bound single-flight gate serializes the context's first resolution without consuming transport capacity. A redirected target enters fast handoff only after a second actual request validates its status and Range. Later Range requests receive the policy-revalidated address without another upstream pre-read for 30 seconds. Redirect rejection and selected transient or authorization failures record a 30-second Adaptive relay decision for that context; HLS resources, users and devices do not share decisions.
11. `TransportPlanner` selects a validated 302 for stable ordinary direct-client files, ordinary relay for server FFmpeg, forced relay or an unstable direct context, and HLS relay for Adaptive or RelayOnly delivery.
12. Ordinary relay streams the upstream response with HTTP status, Range metadata, request cancellation and a full-body idle timeout.
13. HLS recognition uses response metadata and a replayable content prefix. Relay bounds and parses each manifest, replaces URI lines and URI attributes with reusable child capability routes, and relays child manifests, segments, keys and initialization data.
...
- direct-play context decisions: 30 seconds, maximum 4096, memory only
