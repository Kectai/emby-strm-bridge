...
- 文档状态：详细设计，经开源实现对比修订；M5.0 代码准备已实现、实机门槛待验证，M5.2 待实施
...
所有管理 API 仅允许管理员用户会话或管理员显式创建的 Emby 服务器 API Key；非管理员用户会话仍拒绝。改变状态的接口只接受 POST，并支持取消令牌。
...
当前实现状态：已提供管理员会话或管理员创建的服务器 API Key显式确认的内存注册 API、最多 8 条且固定 1 小时失效的 256 位随机能力、可选受限容器后缀、GET/HEAD Gateway、现有重定向策略复用、直达 200/206 拒绝、配置/清理/停服失效和自动化测试。尚未接触用户媒体目录，也未完成本节要求的 Emby 实机证据，因此里程碑仍标记为实验准备而非完成。
